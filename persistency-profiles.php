<?
    ...
    */
    ...
    function show_agent_persistency_dashboard ( $atts ) {
        ...
        $atts = shortcode_atts(array('display_mode' => 'mini'), $atts, 'show_agent_persistency_dashboard');

        /*
        * Standard logic block. Check for agent_id parameter
        * if not set, ask for currently logged in user then pull their meta tags to check for persistency
        * if set, use that agent number to search for matching user then pull their meta tags
        */
        ...
        if (!isset($_GET['agent_id'])) {
            ...
            foreach ($currentUser as $firstUser) {
                $user_by_number = $currentUser[0];
            }
            
            $dashboardAgentNumber = $_GET['agent_id'];
            $agent_is_persistency = get_user_meta($user_by_number->ID, 'is_persistency', true);
        ...
        // // var dump the user variables:
        // print('<h3>Output</h3><pre style="margin-bottom:20px;">');
        // var_dump ( $dashboardAgentNumber );
        // print('<div style="margin-bottom:100px;">');
        // var_dump ( $agent_is_persistency );
        // var_dump ($currentUser);
        // print('</div></pre>');

        // agent_id param works, probably same for currently logged in
        // we want to call the function that prints the 3 boxes
        // or recreate it here and apply the CSS rules from the persistency page
